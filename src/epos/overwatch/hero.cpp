...
static constexpr auto fps = 120.0f;

// Mouse sensitivity in percent.
static constexpr auto sensitivity = 4.0f;
...
void view::sojourn(clock::time_point tp, const epos::input::state& state, const game::scene& scene) noexcept
{
  // Fire lockout duration.
  static constexpr auto fire_lockout = 800ms;

  // Melee lockout duration.
  static constexpr auto melee_lockout = 2000ms;

  // Handle input.
  const auto trigger = state.down(button::right);
  const auto ox = offset(scene.mx, sensitivity);
  const auto oy = offset(scene.my, sensitivity);
  ...
  // Inject fire and schedule melee attack.
  if (fire && state.down(button::right) && tp > fire_lockout_) {
    input_.mask(button::up, 16ms);
    fire_lockout_ = tp + fire_lockout;
  }
  if (melee && tp > melee_lockout_) {
...
void view::widowmaker(clock::time_point tp, const epos::input::state& state, const game::scene& scene) noexcept
{
  ...
  const auto ox = zoom ? offset(scene.mx, sensitivity, relative) : offset(scene.mx, sensitivity);
  const auto oy = zoom ? offset(scene.my, sensitivity, relative) : offset(scene.my, sensitivity);
